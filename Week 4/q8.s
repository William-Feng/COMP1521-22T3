...
# Not strictly necessary, but get into the habit of good style for assignments!
STRUCT_SIZE = 8
INT_SIZE = 4
ROW_OFFSET = 0
COL_OFFSET = 4
...
points_loop_cond:
        ...
        mul     $t4, $t0, STRUCT_SIZE   # offset
        la      $t5, my_points          # &my_points[0]
        add     $t5, $t5, $t4           # &my_points[i]
                                        # my_point[i].row has an offset of 0 with respect to $t5
        lw      $t1, ROW_OFFSET($t5)    # int row = my_points[i].row;
        lw      $t2, COL_OFFSET($t5)    # int col = my_points[i].col;
        
        la      $t5, topography_grid
        mul     $t6, $t1, MAP_SIZE      # row * number of columns
        add     $t6, $t6, $t2           
        mul     $t6, $t6, INT_SIZE      # $t6 = offset
        add     $t6, $t6, $t5
        
        lw      $t3, ($t6)              # int height = topography_grid[row][col];
